...
| [`plan-repl-auto`](skills/plan-repl-auto/SKILL.md)                           | Automated multi-model variant of `plan-repl`: a coordinator fans research out to parallel Sonnet subagents, synthesizes `plan.md` on the session model, and a Fable subagent arbitrates it — an advisory cheap→mid→expensive cascade that replaces the human `> NOTE:` loop. Explicit-only via [`/esond:plan-repl-auto`](commands/plan-repl-auto.md); `--implement` fans out Sonnet subagents to build the approved plan, `--arbiter` overrides the arbiter model. Synthesis runs on the session model (`/model opus`). |
